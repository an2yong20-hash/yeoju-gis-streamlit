...
def _load_bundle() -> dict[str, pd.DataFrame]:
    ...
    dims = load_cached_dims(cache_signature, dim_names=("DimPollingPlace",), columns_map=GIS_DIM_COLUMNS)
    return {
...
def _current_election_frame(df: pd.DataFrame, gusigun_col: str = "구시군명") -> pd.DataFrame:
    ...
    return df.loc[
        df["선거KEY"].astype("string").eq(CURRENT_ELECTION_KEY)
        & df[gusigun_col].astype("string").eq(TARGET_GUSIGUN)
    ].copy()
...
def _render_historical_strategy_summary(votes_df: pd.DataFrame) -> None:
    ...
    st.caption("2018년 자유한국당은 국민의힘 계열로 묶고, 2018년 능서면은 현재 세종대왕면으로 표준화했습니다. 최근 흐름은 2024년 지역구선거 대비 2025년 대통령선거의 양당 격차 변화입니다.")
...
def main() -> None:
    st.title("여주시 전략권역 구분 및 투표구 단위 우선순위 설정")
    st.caption(f"{CURRENT_ELECTION_LABEL} / {TARGET_SIDO} {TARGET_GUSIGUN}")

    try:
        with st.spinner("커스텀 GIS 데이터를 불러오는 중입니다..."):
    ...
    if current_confirmed.empty or current_turnout.empty or current_votes.empty or current_polling.empty:
        st.warning(f"{CURRENT_ELECTION_LABEL} / {TARGET_GUSIGUN} 기준 GIS 데이터를 찾지 못했습니다.")
        st.stop()
    ...
    if all_points:
        ...
        st.download_button(
            "커스텀 GIS 우선순위 투표소 CSV 다운로드",
            dataframe_to_csv_bytes(export_df.loc[:, export_columns]),
            file_name="yeoju_custom_gis_priority_polling_places.csv",
            mime="text/csv",
